...
import { addressesQueryKeys } from '../queryKeys'
...
export function AddressesPage() {
  ...
  const crearMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: addressesQueryKeys.list() }); resetForm() },
  })
  ...
  const eliminarMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: addressesQueryKeys.list() }); setEliminandoId(null) },
    onError: () => setEliminandoId(null),
  })

  const principalMutation = useMutation({
    mutationFn: (id: number) => addressesApi.marcarPrincipal(id),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: addressesQueryKeys.list() }),
  })
