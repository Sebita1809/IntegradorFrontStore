...
export function ProductDetailPage() {
  ...
  const productoQuery = useQuery({
    queryKey: productsQueryKeys.detail(productoId),
  ...
  // Cuantas unidades de este producto ya tenemos en el carrito. Lo usamos para
  // no dejar agregar mas de las que hay en stock.
  const enCarrito = itemsCarrito.find((x) => x.producto_id === p?.id)?.cantidad ?? 0
  const agotado = !p?.disponible || (p?.stock_cantidad ?? 0) <= 0
  const sinMasStock = !!p && enCarrito >= p.stock_cantidad
      ...
      {productoQuery.isLoading ? (
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="space-y-4">
            ...
            >
              {agotado ? 'Producto agotado' : sinMasStock ? 'Sin más stock' : 'Agregar al carrito'}
            </button>
          </div>
        </div>
      )}
