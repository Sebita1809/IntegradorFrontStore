...
import { productsQueryKeys } from '../../products/queryKeys'
...
export function OrderDetailPage() {
  ...
  const pedidoQuery = useQuery({
    queryKey: ordersQueryKeys.detail(pedidoId),
  ...
  // El pedido se actualiza solo cuando el backend avisa un cambio de estado.
  // Apenas conecta, nos suscribimos a este pedido para recibir sus avisos.
  const { conectado, subscribeToOrder } = useOrdersRealtime(Number.isFinite(pedidoId))

  useEffect(() => {
    if (!conectado || !Number.isFinite(pedidoId)) return
    subscribeToOrder(pedidoId)
  }, [conectado, pedidoId, subscribeToOrder])
  ...
  // Si está cancelado, buscamos el motivo en la transición a CANCELADO del historial.
  const cancelado = pedido?.estado_codigo === 'CANCELADO'
  const motivoCancelacion = cancelado
    ? (pedido.historial_estado.find((h) => h.estado_hasta === 'CANCELADO')?.motivo ?? null)
    : null

  async function handleCancelar() {
    try {
      await ordersApi.cancelar(pedidoId, 'Cancelado por el usuario')
      queryClient.invalidateQueries({ queryKey: ordersQueryKeys.detail(pedidoId) })
    } catch {
        ...
        <>
          <div className="rounded-2xl bg-white p-6">
            ...
            <p className="mt-1 text-sm text-gray-500">
              Forma de pago: {FORMA_PAGO_LABEL[pedido.forma_pago_codigo] ?? pedido.forma_pago_codigo}
            </p>
          </div>
